...
const seedData = [
    // Creator 1 content
    {
        id: (0, uuid_1.v4)(), creator_id: CREATOR_1,
    ...
    // Creator 2 content
    {
        id: (0, uuid_1.v4)(), creator_id: CREATOR_2,
    ...
    // Creator 3 content
    {
        id: (0, uuid_1.v4)(), creator_id: CREATOR_3,
...
async function seed() {
    ...
    for (const item of seedData) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
    `).run(item.id, item.creator_id, item.title, item.description, item.category, item.latitude, item.longitude, item.city, JSON.stringify(item.tags), JSON.stringify([PLACEHOLDER_PHOTO]), PLACEHOLDER_TEXT, now);
        inserted++;
