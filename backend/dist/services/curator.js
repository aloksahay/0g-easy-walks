...
exports.curateRoute = curateRoute;
exports.computeContributions = computeContributions;
exports.contributionsToBps = contributionsToBps;
exports.buildGeneratedRoute = buildGeneratedRoute;
 ...
 */
async function curateRoute(items, preferences, startLocation) {
    try {
...
async function aiCurate(items, preferences, startLocation) {
    const durationMap = {
        "1h": 60,
        "2h": 120,
        "3h": 180,
...
Select the best stops and create a walking tour.`;
    const response = await (0, compute_1.chatCompletion)([
 ...
 */
function nearestNeighborFallback(items, preferences, startLocation) {
    const durationMap = {
    ...
    let filtered = items.filter((item) => preferences.categories.includes(item.category));
 ...
 */
function computeContributions(stops) {
    const counts = {};
 ...
 */
function contributionsToBps(contributions) {
 ...
 */
function buildGeneratedRoute(id, curated, itemsById) {
    const stops = curated.selectedIds
        .map((itemId, idx) => {
