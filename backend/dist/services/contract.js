...
exports.buildPurchaseCalldata = buildPurchaseCalldata;
exports.verifyPurchaseTx = verifyPurchaseTx;
exports.getCreatorBalance = getCreatorBalance;
exports.hasPurchased = hasPurchased;
...
function getContract(signerOrProvider) {
    if (!config_1.config.contract.address) {
        throw new Error("CONTRACT_ADDRESS not configured");
    }
    const provider = new ethers_1.ethers.JsonRpcProvider(config_1.config.og.rpc);
    const runner = signerOrProvider || provider;
    return new ethers_1.ethers.Contract(config_1.config.contract.address, ABI, runner);
}
function getSigner() {
    ...
    const provider = new ethers_1.ethers.JsonRpcProvider(config_1.config.og.rpc);
    return new ethers_1.ethers.Wallet(config_1.config.og.privateKey, provider);
}
/**
 * Register a route on-chain. Returns the on-chain route ID.
 */
async function registerRoute(creators, sharesBps, priceWei, routeHash // 66-char hex 0G storage hash
) {
    ...
    const hashBytes32 = routeHash.length >= 66
        ? ("0x" + routeHash.replace("0x", "").slice(0, 64))
        : ethers_1.ethers.zeroPadBytes(routeHash, 32);
    const tx = await contract.registerRoute(creators, sharesBps, BigInt(priceWei), hashBytes32);
 ...
 */
function buildPurchaseCalldata(routeId) {
    const iface = new ethers_1.ethers.Interface(ABI);
 ...
 */
async function verifyPurchaseTx(txHash, routeId, buyerAddress) {
    const provider = new ethers_1.ethers.JsonRpcProvider(config_1.config.og.rpc);
 ...
 */
async function getCreatorBalance(walletAddress) {
 ...
 */
async function hasPurchased(buyerAddress, routeId) {
    const contract = getContract();
