...
async function authWithPrivateKey(baseUrl, privateKey) {
    const wallet = new ethers_1.ethers.Wallet(privateKey);
    const challengeRes = await requestJson(baseUrl, "/api/v1/auth/challenge", {
        method: "POST",
    ...
    const verifyRes = await requestJson(baseUrl, "/api/v1/auth/verify", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
...
async function createContent(baseUrl, token, item) {
    let lastError = "";
    for (let attempt = 1; attempt <= 3; attempt++) {
        const res = await requestJson(baseUrl, "/api/v1/content", {
            method: "POST",
            headers: {
...
async function run() {
    ...
    const generateRes = await requestJson(baseUrl, "/api/v1/routes/generate", {
        method: "POST",
        headers: {
            "content-type": "application/json",
    ...
    const market = new ethers_1.ethers.Contract(config_1.config.contract.address, marketAbi, provider);
    ...
    const prepareRes = await requestJson(baseUrl, `/api/v1/routes/${routeId}/prepare-purchase`, {
        method: "POST",
    ...
    const buyerSigner = new ethers_1.ethers.Wallet(buyerKey, provider);
    const marketWithBuyer = market.connect(buyerSigner);
    ...
    const confirmRes = await requestJson(baseUrl, `/api/v1/routes/${routeId}/confirm-purchase`, {
        method: "POST",
        headers: {
            "content-type": "application/json",
            authorization: `Bearer ${buyer.token}`,
...
//# sourceMappingURL=cannesE2E.test.js.map