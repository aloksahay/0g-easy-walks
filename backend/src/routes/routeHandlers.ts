...
router.post("/:id/store-verification", requireAuth, async (req: Request, res: Response) => {
  const { txHash, creditsNote, sourceMaterials } = req.body as {
    txHash?: string;
    creditsNote?: string;
    sourceMaterials?: Array<{ title: string; author?: string; sourceUrl?: string }>;
  };
  ...
  const purchaseRow = db
    ...
    .get(route.id, req.walletAddress!) as { tx_hash: string; purchased_at: number } | undefined;
  ...
  const itemRows = db
    ...
    .all(...route.item_ids) as Array<{ id: string; title: string; creator_id: string; text_hash: string }>;
  ...
  try {
    const verificationHash = await uploadData(Buffer.from(JSON.stringify(proofPayload), "utf8"));
